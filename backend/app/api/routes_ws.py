...
async def job_websocket(websocket: WebSocket, job_id: uuid.UUID, ticket: str):
    ...
    try:
        async with SessionLocal() as session:
            ...
            # Subscribe before reading the durable backlog. Events created while
            # the backlog is being read will then be received live (and clients
            # de-duplicate the overlap by event id). Reading first used to leave a
            # gap in which short-lived `*.started` and terminal events were lost.
