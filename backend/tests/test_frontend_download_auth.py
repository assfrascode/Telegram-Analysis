...
def test_frontend_download_uses_authorization_header():
    ...
    assert 'fetch(`/jobs/${currentJobId}/report/${path}`' in js
    assert 'downloadArtifact("download-all"' in js
    assert 'job.source_type !== "upload"' in js
...
def test_react_download_uses_content_disposition_filename():
    ...
    assert '`/jobs/${currentJobId}/report/download-all`' in app
    assert 'currentJob.source_type === "upload"' in monitor
